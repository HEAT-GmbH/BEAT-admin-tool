      ...
      {STEPS.map((step, index) => (
        <div key={step.id} className="flex items-stretch gap-2.25">
          <div className="flex flex-col items-center gap-1">
            {completed[`${step.id}`] ? (
              <Icon name="select-box-circle-fill" size={18} color="var(--primary-green)" />
            ) : (
              <div
                className={cn(
          ...
          <div>
            <Link
              href={`${basePath}${step.steps?.length ? step.path + step.steps[0].path : step.path}`}
              onClick={() => startTransition(() => {})}
              className={cn(!step.steps && index !== STEPS.length - 1 && "pb-5.25")}
            >
              <p
                className={cn(
                  "text-base/4.5 text-(--text--sub-600)",
                  activeMainStep?.id === step.id && "text-(--stroke--strong-950)",
            ...
            <div className="flex flex-col gap-2.5">
              {step.steps?.map((subStep, index) => (
                <Link
                  key={subStep.id}
                  href={`${basePath}${step.path + subStep.path}`}
                  onClick={() => startTransition(() => {})}
                ...
                >
                  <div className="px-1 py-1.5 flex items-center gap-1">
                    {!!completed[`${step.id}-${subStep.id}`] && (
                      <Icon name="select-box-circle-fill" size={12} color="var(--primary-green)" />
                    )}
                    <p
                      className={cn(
                    ...
                    <Icon name="arrow-right-up-line" size={12} color="var(--icon--sub-600)" />
                  </div>
                </Link>
              ))}
            </div>
