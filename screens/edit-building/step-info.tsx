...
export const EditStepInfo = () => {
  const { stepBadge, title, description, tip, isPending } = useEditBuilding();
    ...
    <>
      ...
      {!!tip && (
        <div className="flex items-start gap-2 p-2 rounded-[0.5rem] bg-(--state--faded--lighter) text-foreground mb-4">
          <Icon name="information-2-line" color="var(--icon--strong-950)" size={24} />
          <p>{tip}</p>
        </div>
      )}
    </>
